...
Placement and complaints are reputation signals. Tampering is an abuse signal, and it gets its own trigger. Every warmup message carries a single-use token bound to its recipient, so a message cannot be replayed or redirected to inflate anyone's numbers, and a token that lands somewhere it does not belong is simply filed as ordinary mail rather than held against the mailbox that received it. What is held against a mailbox is deleting or spam-flagging warmup mail it received, because only its owner can do that. The [token mechanics](/blog/how-email-warmup-actually-works/) are in the warmup walkthrough.
