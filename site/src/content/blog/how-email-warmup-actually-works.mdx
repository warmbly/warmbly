...
When the receiving side classifies a warmup message, it validates that token on five checks: the HMAC signature, a 48-hour expiry, a single-use ULID so a token can't be replayed, pool membership, and a sender/recipient match. A message that fails these isn't counted as valid warmup. A failed token is not held against the receiving mailbox, which did not put it there; what is held against a mailbox is deleting or spam-flagging warmup mail it received. A mailbox that does that repeatedly gets quarantined, because forged or malformed tokens usually mean someone's trying to inflate warmup numbers without doing the sending.
