...
NATS health probes use a transient durable that starts at the newest message and expires after one minute of inactivity. This prevents concurrent backend replicas from stealing each other's probe while avoiding a permanent consumer for every readiness request.
