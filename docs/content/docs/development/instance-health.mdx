...
Fewer than two warmup pools exist, so warmup cannot place a mailbox and every warmup task fails. Migration `000154` creates the free and premium pools on every instance and nothing else does; a data-only restore or a manual delete removes them. Re-run the migrations or restore the rows. Before that migration a fresh instance had no pools at all, and this was the silent reason warmup never started.
