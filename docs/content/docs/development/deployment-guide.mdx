...
The legacy `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL_TRIAL` / `OPENAI_MODEL_PAID` / `ANTHROPIC_API_KEY` / `AI_LOCAL_MODEL` variables still work as fallbacks, so existing deployments need no change.
