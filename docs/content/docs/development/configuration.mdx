...
| `placement.tests_per_month_trial` | integer, 1 to 100000 | `3` | How many [placement tests](/guides/placement-tests/) a workspace on a free trial may run per calendar month on the instance seed panel. Only an instance running with `DEPLOYMENT_MODE=cloud` meters; a self-hosted instance never counts its own tests. Tests on a workspace's own seed inboxes are never counted |
| `placement.tests_per_month_paid` | integer, 1 to 100000 | `40` | The same allowance for a workspace on a paid plan |
| `placement.seeds_per_test` | integer, 1 to 100 | `40` | The most seed inboxes one test sends to. Every copy is a send from the sending mailbox's daily limit, so this is also how much of that limit one test takes |
| `placement.spacing_seconds` | integer, 5 to 600 | `20` | The average gap between two copies of a test from one mailbox, varied by up to a third either way |
