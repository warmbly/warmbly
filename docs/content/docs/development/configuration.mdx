...
```bash
# The permalinks MaxMind offers on its download page. They always serve the
# current build, so a restart is how an instance picks up a newer one.
GEODB_URL=https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City&license_key=YOUR_KEY&suffix=tar.gz
TRACKING_SCANNER_ASN_DB_URL=https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-ASN&license_key=YOUR_KEY&suffix=tar.gz
```

Any URL serving a MaxMind-format database works, and the shape is read from the content rather than the file name, so a `.tar.gz`, a `.mmdb.gz` and a bare `.mmdb` are all accepted. DB-IP's free [Lite databases](https://db-ip.com/db/lite.php) need no account at all and are published in the same format under `dbip-city-lite` and `dbip-asn-lite`; their country data is comparable and their city data is weaker.
...
A MaxMind permalink carries the account's licence key in its query string. The backend treats both variables as secrets: their values are never returned by any API, the admin panel shows them as set rather than showing them, and the query string is stripped from every log line and error message.
</Callout>
