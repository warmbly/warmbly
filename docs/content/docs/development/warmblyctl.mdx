...
The repository ships skills under `.claude/skills/` (`warmbly-api` for the product, `warmbly-ops` for instance administration) that teach a coding agent these commands, the conventions above, and the sending-safety rules. An agent given a scoped key and those skills can operate a workspace end to end without touching the database.
