...
Default per-check timeout is 3 seconds. Timeouts fail closed: `status=timeout` is a failure, not a skip. The backend coalesces concurrent checks and caches the full matrix for five seconds so a public readiness path cannot create unbounded dependency probes.
...
- `redis-cli -u "$REDIS" GET ops.health.probe`
...
- Point the backend at the previous Redis URL if you just changed `REDIS`.
...
Platform mail transport (login codes, resets) and the provider edge. This plane dials SMTP preflight when `MAIL_TRANSPORT=smtp`. The backend caches the result for five minutes, so frequent readiness requests do not repeatedly authenticate to the relay. It does not send campaign or warmup mail.
