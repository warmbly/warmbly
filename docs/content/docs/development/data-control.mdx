...
If you do want product analytics on your own instance, the variables are on [configuration](/development/configuration/#product-analytics-and-session-replay) and they accept a self-hosted PostHog. It is the same key as the error tracking above, so setting one for errors also turns on the analytics and session replay; `POSTHOG_ERROR_TRACKING` and `WARMBLY_POSTHOG_SESSION_REPLAY` are separate switches if you want only part of it. That is your decision to make, and nothing about it points at us.
