...
### Standing of removed mailboxes

When a mailbox that warmup had penalised is removed, its address and standing (score, state, and any block) are kept for 90 days after removal or after the block ends, whichever is later, so adding it back does not start it clean. A mailbox in good standing leaves nothing behind. This window is fixed rather than a setting, and the rows are purged by the same sweep.
