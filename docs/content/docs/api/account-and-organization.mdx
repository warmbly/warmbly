...
Advertises which native-app social sign-in options this deployment supports, so the mobile app adapts to hosted and self-hosted backends without a rebuild. Client IDs are public identifiers, not secrets.
...
```json
{
  "apple": { "enabled": true, "client_id": "com.warmbly.app" },
...
Exchanges a native Sign in with Apple identity token for a session. The token is verified against Apple's published keys (signature, issuer, audience, expiry). A provider-verified identity is strong auth, so there is no email code step. The first sign-in creates the account, organization, and free trial.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `identity_token` | string | yes | The `identityToken` from the Apple authorization credential. |
| `first_name` | string | no | Apple shares the name only with the app, never inside the token; forwarded for first-sign-in profile prefill. |
| `last_name` | string | no | See `first_name`. |
...
Exchanges a native Google Sign-In ID token for a session. The token is verified against Google's published keys and must be issued for the client ID advertised by `/auth/providers`. The first sign-in creates the account, organization, and free trial.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id_token` | string | yes | The ID token from Google Sign-In. |
