...
`name` is the event name you chose and `payload` is your fields. Subscribe with an API key carrying `REALTIME_SUBSCRIBE` and match on `name` (or use the `CUSTOM` intent to receive only these). This is the recommended way to "tell my system this happened" — no inbound endpoint, no SSRF surface, and it works behind a firewall.
