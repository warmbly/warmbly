...
| `placement_daily_budget` | 409 | The test needs more sends than the mailbox has left of its daily limit today. The `message` gives both numbers. Every copy is a send from that limit |
