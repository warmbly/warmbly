...
An API key is also how an [MCP client](/api/mcp/) (Claude Code, Claude Desktop, Cursor) authenticates to Warmbly. Send the key as a bearer token to `https://api.warmbly.com/v1/mcp`; the client then sees exactly the tools the key's scopes allow.
