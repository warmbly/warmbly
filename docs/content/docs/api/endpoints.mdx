...
| GET | `/ai/sessions/:id/messages` | organization member |
...
`POST /v1/mcp` exposes the tool registry over the [Model Context Protocol](/api/mcp/) streamable-HTTP transport. It is API-key only, each tool is gated by its scope, `tools/list` reflects only what the key allows, and send-class tools are never exposed. Per-key rate limits apply.
