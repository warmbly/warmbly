...
Placement history is kept for the life of the mailbox, like warmup volume history, and moves with a workspace export. Deliveries before this report existed are counted from the receipts still on file, with category tabs and rescues reported as `0` for that period.
