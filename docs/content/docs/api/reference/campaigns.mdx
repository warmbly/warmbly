---
title: Campaigns
description: Create campaigns and sequences, manage senders, ramp and tracking settings, A/B variants, attachments, preflight, and start or stop sends.
icon: Megaphone
---
```

Let me confirm the Search `q`/`folder` query params and that `Search` always returns the `CampaignsResult` envelope (data + Pagination). Already confirmed from handler and model. Now writing the final output.
