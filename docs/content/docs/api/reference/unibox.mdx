...
Forwarding reads the message, so an API key restricted to certain mailboxes must be allowed the mailbox the message belongs to as well as the sending mailbox; otherwise the request answers `403`. A `forward_message_id` that names no message in the organization answers `404`.
