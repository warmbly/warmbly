...
| `offset` | query | int | Row offset (clamped to >= 0). |

### Response

An offset-paginated `data` + `pagination` envelope. Note this uses `offset` / `total`, not the opaque-cursor shape used by other list endpoints.
