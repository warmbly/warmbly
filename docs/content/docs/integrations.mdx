...
- A `static` value can include event variables. For example a static company source written as `{{.company}}` is filled from the event data. See [Personalization & expressions](/expressions) for the full variable list.
...
Integrations are most useful as the actions at the end of an automation. An [automation](/automations) is a flow that starts with a trigger event and runs one or more action steps. Your connected integrations are what those steps act on:
...
You can also filter when an action runs. A common pattern is to only notify or push to a CRM on a `positive` reply, or only above a minimum classifier confidence. A reply event also informs the [deliverability](/deliverability) signals Warmbly tracks per mailbox.
