...
| Seed test placement by provider | The seed split per Gmail, Outlook, and IMAP/SMTP |
...
**Seed test placement by provider is the check before scaling a campaign.** An aggregate rate can look fine while one provider quietly junks everything: Gmail at `95%` inbox with Outlook at `40%` is a provider-specific problem, usually authentication or content, not volume.
