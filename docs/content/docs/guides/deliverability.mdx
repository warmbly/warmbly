...
| Warmup inbox placement | Where warmup mail landed each day (inbox, Gmail tabs, spam), the inbox rate over time, the split per recipient provider and each mail host inside it, and every mailbox ranked by its seven-day inbox rate |
...
3. Check placement by provider, and the hosts inside each one, to see whether mail actually reaches inboxes.
