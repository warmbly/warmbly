...
Campaign sequences have their own AI step that runs per contact instead of per event: you write an instruction, draw named paths out of the step, and the model routes each contact down exactly one of them. See [Sequences](/guides/sequences/#ai-steps).
