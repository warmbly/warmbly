...
Campaign sequences have their own AI step that runs per contact instead of per event: it can label the contact (and route on the label with an "AI label is …" branch) and fill contact custom fields for later emails. See [Sequences](/guides/sequences/#ai-steps).
