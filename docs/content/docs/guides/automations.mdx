...
Three special conditions are available for any trigger:

- **Ask AI (yes/no)** puts a model in the branch: you write a plain-language question about the event ("Is this reply asking about pricing?"), and the answer picks the path — yes takes the right dot, no takes the bottom dot. The question is templated, so you can drop event fields into it with `{{.field}}`. Each evaluation costs one credit; if the model can't answer (or you are out of credits), the no path runs and the run history shows why. See [AI steps in automations](/guides/ai-steps-in-automations/).
...
AI steps are the one exception: a Test runs them for real so you can see the actual model output and the branches it drives, which means a Test spends one credit per AI step. See [AI steps in automations](/guides/ai-steps-in-automations/).
