...
Only a person answering you can opt out this way: someone replying in one of your campaign threads, or one of your contacts writing to you. Everything else that lands in a mailbox is left alone, even when it says "unsubscribe":

- a bounce or delivery failure, which returns your own email (and its unsubscribe header) inside the notice
- an out-of-office or other automatic reply
- a newsletter or notification sent to a list, whose "unsubscribe" footer is its own sender's

Entries written under an earlier, broader reading of replies are read again once. When neither the message that wrote an entry nor any other message from that address asks to stop under this rule, the entry is lifted, the contact's **Subscribed** flag is restored, and the lift is recorded in the audit log. An entry that began with a link or one-click unsubscribe, or whose mail can no longer be found, stays.
