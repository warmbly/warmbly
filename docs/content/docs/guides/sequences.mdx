...
The reverse holds too. A step written through the API or an agent tool usually carries `body_plain` and nothing else, and the HTML half is rendered from that text: the lines are kept and bare URLs become links, so click tracking has something to wrap. Without it the step would arrive blank, because every modern client prefers the HTML alternative and there was none to show. A step that already has an HTML body is never overwritten.
