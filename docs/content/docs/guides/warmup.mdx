...
A block also holds for its full term. The signals behind it age out of their windows long before the block ends, and that does not release it early.

The standing follows the address, not the connection. Removing a mailbox from the workspace and adding it back does not clear its score or its block: for 90 days after removal, or after the block ends, whichever is later, the same address rejoins the pool with the standing it left with. A mailbox in good standing leaves nothing behind.
