...
Chat respects record permissions: a message typed on a surface is only delivered to teammates whose role can see that kind of record, enforced by the realtime service like the rest of the event stream.
