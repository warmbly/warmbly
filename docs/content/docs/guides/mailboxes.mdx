...
SMTP accepts port `465` (implicit TLS, the connection is encrypted from the first byte) or `587` (STARTTLS, upgraded in-band); both are fully supported for validation and sending. IMAP always connects over implicit TLS regardless of port, with `993` the common choice.
