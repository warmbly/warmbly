...
| Placement test sends | Copies of a test that had not been sent yet stay behind, so the destination never sends to the source instance's seeds. Tests and their results arrive as a record. A campaign's placement monitor keeps its settings and next run, but not the record of its last run |
