...
The panel is yours to arrange: drag its inner edge to resize, and move it to the left or right edge of the screen from the header (both remembered). Inside the panel, `Ctrl/Cmd + ]` and `[` switch tabs, `Alt + N` opens a new chat, `Alt + W` closes the tab, and `Alt + M` minimizes; the full list is in the shortcuts dialog (`?`).
