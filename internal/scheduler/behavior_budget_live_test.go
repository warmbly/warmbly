...
func liveBehavior(t *testing.T, handle *db.DB, pool *pgxpool.Pool) behavior.Service {
	t.Helper()
	return behavior.NewService(
		repository.NewBehaviorRepository(pool),
...
func TestLiveProfileBudgetMatchesTheFixedScheduleCounter(t *testing.T) {
	...
	f.addSentLead(t)
	confirmed := f.completeCampaignTask(t, nil, nil, earlierToday(time.Hour))
