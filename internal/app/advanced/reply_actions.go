...
func (s *service) fireInstantActions(ctx context.Context, campaignID, contactID, currentStepID uuid.UUID, eventKind string) {
	...
	for hops := 0; hops < maxInstantChain; hops++ {
		...
		// scheduler resume the contact after the wait. "ai" also hands back to
		// the scheduler: AI steps run only through the scheduler path, where the
		// provider + credit ledger live (the node is NOT stamped sent here, so
		// the scheduler routes to it and runs it at the next boundary). "end"
		// terminates the chain.
		if cfg.Type == "wait" || cfg.Type == "ai" {
			return
		}
