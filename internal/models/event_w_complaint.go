...
type JobEventInboundComplaint struct {
	...
	OriginalMessageID string `json:"original_message_id" avro:"original_message_id"`
	// ComplainedRecipient is the address that reported it; several providers
	// redact this, so it is often empty.
	ComplainedRecipient string `json:"complained_recipient" avro:"complained_recipient"`
	// Provider is the reporting agent, for the event record.
	Provider string `json:"provider" avro:"provider"`
}
