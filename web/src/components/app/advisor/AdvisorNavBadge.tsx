...
export default function AdvisorNavBadge({ surface }: { surface: AdvisorSurface }) {
    ...
    return (
        <motion.span
            ...
            title={label}
            aria-label={label}
            className={`inline-flex h-4 min-w-[16px] items-center justify-center rounded-full px-1 text-[10px] font-medium tabular-nums ${
                counts.critical > 0 ? "bg-rose-500 text-white" : "bg-amber-100 text-amber-800"
            }`}
        >
