// ContactRecipientField — the compose "To" editor: chip-based like the reply
// composer's RecipientField, plus a live contact autocomplete under the input.
// Typing searches the org's contacts (name, email, company); Enter/Tab/","
// still commits raw addresses so recipients outside the CRM work too.
...
export default function ContactRecipientField({
    ...
    const showMenu = focused && query.length > 0 && suggestions.length > 0;
                        ...
                        {suggestions.map((c, i) => (
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    <span className="block text-[12px] font-medium text-slate-900 truncate leading-tight">
                                        {contactName(c)}
                                        {c.company && (
