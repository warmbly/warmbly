...
- `>= 3` invalid warmup-token attempts in `24h` (no live path records one since #481; the band is unreachable until a signal can attribute a forged token to its sender)
- spam score `> 50`
